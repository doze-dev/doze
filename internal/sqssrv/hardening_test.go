...
func TestDedupGCAndExpiry(t *testing.T) {
	...
	if _, err := s.CreateQueue("d.fifo", map[string]string{"FifoQueue": "true"}); err != nil {
		t.Fatal(err)
	}
...
func TestRetentionSweep(t *testing.T) {
	...
	if _, err := s.CreateQueue("q", map[string]string{"MessageRetentionPeriod": "1"}); err != nil {
		t.Fatal(err)
	}
...
func TestLongPollWakesPromptly(t *testing.T) {
	s := testStore(t)
	if _, err := s.CreateQueue("q", nil); err != nil {
		t.Fatal(err)
	}
	go func() {
